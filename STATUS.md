...
Gates at reconciliation: `pnpm test` 72/72 (covers `features` and `lib`), `pnpm test:db`
54/54 (Neon), `pnpm lint` clean, `tsc --noEmit` clean, production build green. The build only passes with `TRANSACTIONAL_EMAIL_PROVIDER`
...
5. ~~**PLAN.md Phase 3.**~~ Done 2026-09-02 (see Done). Phases 4–5 (governance:
   templates, models, quotas, budgets; customer experience) remain, as does the output
   moderation gate (G10) and cost evidence (G8) from Phase 3.
